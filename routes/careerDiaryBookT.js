...
// 先取出日記tag資料
  ...
  pool.query('select a.*,b.*,c.tagName from diary a,tmember b, diarytag c where a.tagNo=? and a.memNo=? and b.memNo=? and c.tagNo=?', [tagNo,memNo,memNo,tagNo], function(err, results) {
    if(results.length==0){
      ...
      res.render('careerDiaryBookT', {memNo:req.session.memNo, memTitle:req.session.memTitle,picture:req.session.picture,data:results,tagNo:tagNo,diaryTagDat:diaryTagData});
